...
try
    ...
    [File_filename,File_path] = uigetfile('*.csv','< Load the in logging data >',Logging_csv_path);
catch
    % File path select from ui window
    [File_filename,File_path] = uigetfile('*.csv','< Load the in logging data >');
end;

Logging_csv_path = [File_path, File_filename];
save('PreviousPath','Logging_csv_path');
...
for idxA1 = 1:1:nA1
    for idxZoe = idxZoe_start:1:nZoe
        if(A1.UTCTime(idxA1) == Zoe.UTCTime(idxZoe))
            ...
            Ref_Lat = Zoe.Lat(10); Ref_Lon = Zoe.Lat(10);
            Lat = Zoe.Lat(idxZoe); Lon = Zoe.Lat(idxZoe); Hgt = 0;
            ...
            Ref_Lat = Zoe.Lat(idxZoe); Ref_Lon = Zoe.Lat(idxZoe);
            Lat = A1.Lat(idxA1); Lon = A1.Lat(idxA1); Hgt = 0;
            enu = FnFast_llh2enu(Ref_Lat,Ref_Lon,Lat,Lon,Hgt);
...
destination = 'C:\Users\A1\Desktop\SensorAnalysis\01_Matlab_Sangwoo\DB\Result\';
